...
trait MachinePropertyTraits<T extends GroovyTileBasic> implements IMachineProperties<T> {
    ...
    void registerTileEntity() {
        //TileEntityType.Builder.create(getTeClass().&new, block).build(null).setRegistryName("groovymc", name);
        EventRegistry.addToTileEntityTypeList(getTeClass().getConstructor().newInstance(), getBlock(), "groovymc", getName());
    }

    @Override
    void registerTileEntity(String modID) {
        //TileEntityType.Builder.create(getTeClass().&new, block).build(null).setRegistryName(modID, name);
        EventRegistry.addToTileEntityTypeList(getTeClass().getConstructor().newInstance(), getBlock(), getName());
    }

    @Override
    void registerTileEntity(TileEntity tile) {
        EventRegistry.addToTileEntityTypeList(tile, block, name);
    }
